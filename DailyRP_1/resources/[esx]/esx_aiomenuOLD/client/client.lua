...
AddEventHandler('sendProximityMessageID', function(id, message)
  ...
  if pid == myId then
    TriggerEvent("chatMessage", "[DOWOD]" .. "", {0, 153, 204}, "^7 " .. message)
  elseif GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(myId)), GetEntityCoords(GetPlayerPed(pid)), true) < 4.999 then
    TriggerEvent("chatMessage", "[DOWOD]" .. "", {0, 153, 204}, "^7 " .. message)
  end
end)
...
AddEventHandler('sendProximityMessagePhone', function(id, name, message)
  ...
  local pid = GetPlayerFromServerId(id)
  if pid == myId then
    TriggerEvent("chatMessage", "[TELEFON]^3(" .. name .. ")", {0, 153, 204}, "^7 " .. message)
  elseif GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(myId)), GetEntityCoords(GetPlayerPed(pid)), true) < 4.999 then
    TriggerEvent("chatMessage", "[TELEFON]^3(" .. name .. ")", {0, 153, 204}, "^7 " .. message)
  end
end)