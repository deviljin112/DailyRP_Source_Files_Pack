...
TriggerEvent('esx_society:registerSociety', 'motodealer', 'Concessionnaire', 'society_motodealer', 'society_motodealer', 'society_motodealer', {type = 'private'})
...
AddEventHandler('onMySQLReady', function ()
  Categories       = MySQL.Sync.fetchAll('SELECT * FROM motos_categories')
  local vehicles   = MySQL.Sync.fetchAll('SELECT * FROM motos')
...
RegisterServerEvent('esx_motoshop:setVehicleOwned')
AddEventHandler('esx_motoshop:setVehicleOwned', function (vehicleProps)
...
RegisterServerEvent('esx_motoshop:setVehicleOwnedPlayerId')
AddEventHandler('esx_motoshop:setVehicleOwnedPlayerId', function (playerId, vehicleProps)
...
RegisterServerEvent('esx_motoshop:setVehicleOwnedSociety')
AddEventHandler('esx_motoshop:setVehicleOwnedSociety', function (society, vehicleProps)
  local _source = source
  local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('esx_motoshop:sellVehicle')
AddEventHandler('esx_motoshop:sellVehicle', function (vehicle)
  MySQL.Async.fetchAll(
    'SELECT * FROM motodealer_motos WHERE vehicle = @vehicle LIMIT 1',
    { ['@vehicle'] = vehicle },
    function (result)
      ...
      MySQL.Async.execute(
        'DELETE FROM motodealer_motos WHERE id = @id',
        { ['@id'] = id }
      )
    end
  )
...
RegisterServerEvent('esx_motoshop:rentVehicle')
AddEventHandler('esx_motoshop:rentVehicle', function (vehicle, plate, playerName, basePrice, rentPrice, target)
...
RegisterServerEvent('esx_motoshop:setVehicleForAllPlayers')
AddEventHandler('esx_motoshop:setVehicleForAllPlayers', function (props, x, y, z, radius)
  TriggerClientEvent('esx_motoshop:setVehicle', -1, props, x, y, z, radius)
end)
...
ESX.RegisterServerCallback('esx_motoshop:buyVehicle', function (source, cb, vehicleModel)
  local xPlayer     = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_motoshop:buyVehicleSociety', function (source, cb, society, vehicleModel)
  local vehicleData = nil
  ...
  TriggerEvent('esx_addonaccount:getSharedAccount', 'society_' .. society, function (account)
    if account.money >= vehicleData.price then
      ...
      MySQL.Async.execute(
        'INSERT INTO motodealer_motos (vehicle, price) VALUES (@vehicle, @price)',
        {
...
ESX.RegisterServerCallback('esx_motoshop:getPersonnalVehicles', function (source, cb)
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_motoshop:getCommercialVehicles', function (source, cb)
  MySQL.Async.fetchAll(
    'SELECT * FROM motodealer_motos ORDER BY vehicle ASC',
    {},
    function (result)
      local vehicles = {}
...
ESX.RegisterServerCallback('esx_motoshop:resellVehicle', function (source, cb, plate, price)
  MySQL.Async.fetchAll(
    'SELECT * FROM rented_vehicles WHERE plate = @plate LIMIT 1',
    { ['@plate'] = plate },
    function (result)
...
if Config.EnablePvCommand then
  TriggerEvent('es:addCommand', 'pv', function (source, args, user)
    TriggerClientEvent('esx_motoshop:openPersonnalVehicleMenu', source)
  end, {help = _U('leaving')})
end
