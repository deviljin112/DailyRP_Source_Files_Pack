...
local home = {x=2671.75,y=1600.820,z=24.85}
local homeblip = {x=2671.75,y=1600.820,z=24.85}
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if(isNear(tenue)) then
				Info("Wcisnij ~g~E~w~ aby sie przebrac.")
		...
		if(isNear(gamestart)) then
				Info("Wcisnij ~g~E~w~ aby rozpoczac gre.")
				if(IsControlJustPressed(1, 38)) then
					debutpartie()
				end
		end
		...
		if(isNear(miseazero)) then
				Info("Wcisnij ~g~E~w~ aby zresetowac punktacje.")
				if(IsControlJustPressed(1, 38)) then
					remettrezero()
				end
		end
		...
		if (Vdist(pos.x, pos.y, pos.z, home.x, home.y, home.z) < 2.0) then
				Info("Wcisnij ~g~E~w~ aby isc na pole do paintballu.")
				if(IsControlJustPressed(1, 38)) then
					allerjouer()
				end
		end
		...
		if (Vdist(pos.x, pos.y, pos.z, activity.x, activity.y, activity.z) < 2.0) then
				Info("Wcisnij ~g~E~w~ aby wyjsc z pola do paintballu.")
				if(IsControlJustPressed(1, 38)) then
					quitterpaintball()
				end
		end
...
function OuvrirVestiaire()
  ...
  local elements = {
    ...
    table.insert(elements, {label = "Druzyna 1", value = 'tenue_equipe'})
	  table.insert(elements, {label = "Druzyna 2", value = 'tenue_equipedeux'})
...
function debutpartie()
	...
	if(distancenotification(millieu)) then
		...
		ESX.ShowNotification('~p~Gra zaczyna sie za ~r~~h~3')
		Citizen.Wait(1000)
		ESX.ShowNotification('~p~Gra zaczyna sie za ~r~~h~2')
		Citizen.Wait(1000)
		ESX.ShowNotification('~p~Gra zaczyna sie za ~r~~h~1')
		Citizen.Wait(1000)
		ESX.ShowNotification('~p~Gra ~r~~h~START !!')
	end
end
...
Citizen.CreateThread(function()
    function Initialize(scaleform)
        ...
        PushScaleformMovieFunctionParameterString("~r~PAINTBALL !!")
	    	PushScaleformMovieFunctionParameterString("Mam nadzieje ze masz bron ze soba !")
...
ESX.ShowNotification('~p~Punkty zresetowane')
end
