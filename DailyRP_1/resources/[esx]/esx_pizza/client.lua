  ...
  while true do
    ...
    if isInJobPizz == false then
      ...
      if GetDistanceBetweenCoords(pizzeria.x, pizzeria.y, pizzeria.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        HelpText("Nacisnij ~INPUT_CONTEXT~ aby rozpoczac dostawe ~r~pizzy",0,1,0.5,0.8,0.6,255,255,255,255)
    ...
    if isToHouse == true then
      ...
      while notif == true do
        ...
        TriggerEvent("pNotify:SendNotification", {
          text = "Jedz do : " ..destinol.. " aby dostarczyc pizze",
          type = "info",
          queue = "global",
          timeout = 4000,
          layout = "bottomRight"
      ...
      if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
        HelpText("Wcisnij ~INPUT_CONTEXT~ aby dostarczyc pizze",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsControlJustPressed(1,38) then
          ...
          if (posibilidad > 70) and (posibilidad < 90) then
            ...
            pourboire = math.random(50, 250)

            TriggerEvent("pNotify:SendNotification", {
              text = "Masz napiwek : " .. pourboire .. "$",
              type = "success",
              queue = "global",
              timeout = 4000,
              layout = "bottomRight"
    ...
    if isToPizzaria == true then
      ...
      if GetDistanceBetweenCoords(pizzeria.x,pizzeria.y,pizzeria.z, GetEntityCoords(GetPlayerPed(-1),true)) < 3 and afaitunepizzamin == true then
        HelpText("Nacisnij ~INPUT_CONTEXT~ aby wziasc pizze",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("faggio2"))  then
          ...
          if IsControlJustPressed(1,38) then
            ...
            if IsInVehicle() then
              ...
              TriggerEvent("pNotify:SendNotification", {
                text = "Dziekujemy za twoja prace, oto twoja wyplata : " .. paie .. "$",
                type = "success",
                queue = "global",
                timeout = 4000,
                layout = "bottomRight"
  ...
  while true do
    ...
    if isInJobPizz == true then
      ...
      if GetDistanceBetweenCoords(pizzeriafin.x, pizzeriafin.y, pizzeriafin.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        HelpText("Nacisnij ~INPUT_CONTEXT~ aby zatrzymac dostawe ~r~pizzy",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsControlJustPressed(1,38) then
          ...
          if afaitunepizzamin == true then
            ...
            TriggerEvent("pNotify:SendNotification", {
              text = "Dziekuje za prace, dowidzenia",
              type = "success",
              queue = "global",
              timeout = 4000,
              layout = "bottomRight"
