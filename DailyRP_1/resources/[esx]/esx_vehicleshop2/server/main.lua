...
-- unused?
RegisterServerEvent('esx_vehicleshop2:setVehicleForAllPlayers')
AddEventHandler('esx_vehicleshop2:setVehicleForAllPlayers', function (props, x, y, z, radius)
	TriggerClientEvent('esx_vehicleshop2:setVehicle', -1, props, x, y, z, radius)
end)
...
ESX.RegisterServerCallback('esx_vehicleshop2:getPersonnalVehicles', function (source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('esx_vehicleshop2:returnProvider', function(vehicleModel)
	...
	MySQL.Async.fetchAll('SELECT * FROM cardealer_vehicles2 WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			print('esx_vehicleshop2: ' .. GetPlayerIdentifiers(_source)[1] .. ' attempted selling an invalid vehicle!')
		end
...
ESX.RegisterServerCallback('esx_vehicleshop2:resellVehicle', function (source, cb, plate, price)
	MySQL.Async.fetchAll('SELECT * FROM rented_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function (result)
...
if Config.EnablePvCommand then
	TriggerEvent('es:addGroupCommand', 'pv', 'user', function(source, args, user)
		TriggerClientEvent('esx_vehicleshop2:openPersonnalVehicleMenu', source)
	end, {help = _U('leaving')})
end
