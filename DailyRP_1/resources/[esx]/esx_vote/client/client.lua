...
function OpenVoteMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'vote',
		{
			title    = "Candidats",
			elements = {
				{label = "Kandydat 1", value = 'candidat1'},
				{label = "Kandydat 2", value = 'candidat2'},
				{label = "Glosowanie", value = nil}
			}
		},
		function(data, menu)
			TriggerServerEvent("elec:vote",data.current.value)
			TriggerEvent('esx:showNotification', "Zaglosowales na "..data.current.label)
			menu.close()
		end,
		function(data, menu)
			menu.close()
	...
	while true do
		...
		for k,v in pairs(Config.Zones) do
			if(GetDistanceBetweenCoords(coords, v.MarkerPos.x, v.MarkerPos.y, v.MarkerPos.z, true) < v.Size.x) then
				drawTxt('Wcisnij ~g~E~s~ aby ~b~zaglosowac', 2, 1, 0.5, 0.8, 0.6, 255, 255, 255, 255)
