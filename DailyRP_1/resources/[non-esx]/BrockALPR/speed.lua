...
local radar =
{
	shown = false,
	freeze = false,
	info = "~y~System LSPD Radar...~w~321...~y~Zaladowany! ",
	info2 = "~y~System LSPD Radar...~w~321...~y~Zaladowany! ",
	minSpeed = 5.0,
	maxSpeed = 75.0,
}
...
Citizen.CreateThread( function()
	...
	while true do
		...
		if IsControlJustPressed(1, 128) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) then
			...
			if radar.shown then 
				radar.shown = false 
				radar.info = string.format("~y~System LSPD Radar...~w~321...~y~Zaladowany! ")
				radar.info2 = string.format("~y~System LSPD Radar...~w~321...~y~Zaladowany! ")
			else 
				radar.shown = true 
			end		
                Wait(75)
		...
		if radar.shown  then
			if radar.freeze == false then
					...
					if IsEntityAVehicle(e) then
						...
						radar.info = string.format("~y~Rejestracja: ~w~%s  ~y~Model: ~w~%s  ~y~Predkosc: ~w~%s mph", fplate, fmodel, math.ceil(fvspeed))
					end
					...
					if IsEntityAVehicle(j) then
						...
						radar.info2 = string.format("~y~Rejestracja: ~w~%s  ~y~Model: ~w~%s  ~y~Predkosc: ~w~%s mph", bplate, bmodel, math.ceil(bvspeed))
			...
			DrawAdvancedText(0.591, 0.903, 0.005, 0.0028, 0.4, "Przedni Radar", 0, 191, 255, 255, 6, 0)
			DrawAdvancedText(0.591, 0.953, 0.005, 0.0028, 0.4, "Tylni Radar", 0, 191, 255, 255, 6, 0)
