    	...
    	if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.ATMS[i].x, Config.ATMS[i].y, Config.ATMS[i].z, true) < 2  then
			...
			time = 0				
				if ( Config.ATMS[i].b == nil ) then
					inMarker = true
					hintToDisplay('Wcisnij ~INPUT_PICKUP~ aby uzyc bankomatu')
				else
					inBankMarker = true
					type = Config.ATMS[i].t
					hintToDisplay('Wcisnij ~INPUT_PICKUP~ aby skorzystac z okienka')
				end
...
RegisterNUICallback('error', function(data, cb)
	...
	open = false
	cb('ok')
	ESX.ShowNotification('Maszyna wczytuje, prosze czekac...')
end)
