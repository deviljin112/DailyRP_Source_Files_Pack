...
AddEventHandler('jsfour-atm:insert', function(amount)
	...
	if amount > xPlayer.getMoney() then
		...
		TriggerClientEvent('esx:showNotification', _source, 'Wplaciles ' .. amount .. '~s~$')
	end
end)
...
AddEventHandler('jsfour-atm:take', function(amount)
	...
	if amount > accountMoney then
		...
		TriggerClientEvent('esx:showNotification', _source, 'Wyplaciles ' .. amount .. '~s~$')
	end
end)
  ...
  function (result)
    if (result[1] ~= nil) then
      ...
      local senPlayer    = ESX.GetPlayerFromId(_source)
        ...
        function (result)
          if (result[1] ~= nil) then
            TriggerClientEvent('esx:showNotification', _source, 'Przelales ' .. amount .. '~s~$ do ' .. result[1].firstname .. ' ' .. result[1].lastname)
            TriggerClientEvent('esx:showNotification', recPlayer.source, 'Dostales ' .. amount .. '~s~$ z przelewu od ' .. result[1].firstname .. ' ' .. result[1].lastname)
          end
        end)
      end
    end
...
AddEventHandler('jsfour-atm:createAccount', function( src )
  ...
  local account = '2077-6, ' .. math.random(0,9) .. ' ' .. math.random(000,999) .. ' ' .. math.random(000,999) .. '-' ..math.random(0,9)
